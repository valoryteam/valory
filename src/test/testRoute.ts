...
export class TestRoute extends Controller {
	...
	public submitGeneric(@Body() input: GenericType<Item>) {
		input.generic = {...input.generic, id: "stuff"};
		return input;
	}

	@Post("submit/generic/literal")
	public submitGenericLiteral(@Body() input: {literal1: GenericType<{potato: boolean}>, literal2: GenericType<{other: string}>}) {
		return input;
	}

	@Post("submit/indexed")
	public submitIndexedAccess(@Body() input: ItemRef) {
		return input;
	}
}
